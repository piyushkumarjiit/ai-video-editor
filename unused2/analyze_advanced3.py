...
def load_qwen_vl_model(model_name, device=None, torch_dtype=None, quantization=None):
    ...
    try:
        ...
        # ONLY LLaVA-1.6-vicuna-13B-Q4_K_M
        model_configs = [
            {
                "type": "llava",
                "name": "LLaVA-1.6-vicuna-13B-Q4_K_M",
                "pattern": "models--cjpais--llava-v1.6-vicuna-13b-gguf/**/llava-v1.6-vicuna-13b.Q4_K_M.gguf",
        ...
        for config in model_configs:
            matches = list(cache_dir.glob(config["pattern"]))
            if matches:
                model_path = str(matches[0])
                model_type = config["type"]
                n_ctx = config["n_ctx"]
                
                # Check for mmproj if LLaVA model
                if model_type == "llava":
                    mmproj_matches = list(cache_dir.glob(config["mmproj_pattern"]))
                    if mmproj_matches:
                        mmproj_path = str(mmproj_matches[0])
                        print(f"   Model: {config['name']} (13B parameters)")
                        break
                    else:
                        # LLaVA needs mmproj, skip if not found
                        model_path = None
                        continue
                else:
        ...
        if not model_path:
            ...
            print('   Qwen2-VL-7B: python -c "from huggingface_hub import hf_hub_download; hf_hub_download(repo_id=\'bartowski/Qwen2-VL-7B-Instruct-GGUF\', filename=\'Qwen2-VL-7B-Instruct-Q5_K_M.gguf\')"')
            print('   LLaVA-13B: python -c "from huggingface_hub import hf_hub_download; hf_hub_download(repo_id=\'cjpais/llava-v1.6-vicuna-13b-gguf\', filename=\'llava-v1.6-vicuna-13b.Q4_K_M.gguf\'); hf_hub_download(repo_id=\'cjpais/llava-v1.6-vicuna-13b-gguf\', filename=\'mmproj-model-f16.gguf\')"')
            return None
        ...
        if model_type == "llava":
            # Load LLaVA model with vision projector (logging disabled at C++ level)
            chat_handler = Llava16ChatHandler(clip_model_path=mmproj_path, verbose=False)
            llm = Llama(
                model_path=model_path,
                chat_handler=chat_handler,
                n_gpu_layers=-1,
                n_ctx=n_ctx,  # 3072 for LLaVA (images need ~2900 tokens)
                n_threads=8,
                verbose=False,
                logits_all=False,
            )
        ...
        model_display = "LLaVA-1.6-vicuna-13B" if model_type == "llava" else Path(model_path).stem
        print(f"   ✓ {model_display} GGUF loaded on GPU in {load_time:.1f}s")
        print(f"   Context: {n_ctx} tokens, All layers on GPU")
        ...
        return {
            ...
            "device": "cuda",
            "kind": "qwen2-vl-gguf" if model_type == "qwen2-vl" else "llava-gguf",
            "model_path": model_path
        }
    except Exception as e:
...
def caption_keyframes(frames, caption_model, max_length=40, num_beams=5, prompt=None, use_prompt_for_blip=False, min_length=None, max_image_size=None, multi_frame_context=True):
    """Generate captions for keyframes with multi-frame context for better scene understanding."""
    ...
    # Group frames by scene for multi-frame analysis
    frames_by_scene = {}
    for frame in frames:
    ...
    model_display = caption_model.get("model_name", "AI Model")
    
    print(f"\n📝 Captioning with {model_display}")
    print(f"   Total: {total_frames} kept frames across {total_scenes} scenes")
    print(f"   Progress will be shown for each scene...")
    
    # GGUF model doesn't need torch.no_grad()
    use_torch_guard = model_kind not in {"qwen2-vl-gguf", "llava-gguf"}
    
    # Track progress outside nested function so it's accessible later
    scene_count = 0
    frame_count = 0
    import time
    ...
    def process_frames():
        nonlocal scene_count, frame_count
        
        # Process each scene with 2-3 frames for better context
        for scene_id in sorted(frames_by_scene.keys()):
            scene_frames = frames_by_scene[scene_id]
            scene_count += 1
            scene_start_time = time.time()
            
            # GGUF model (llama-cpp-python) - Qwen2-VL supports multi-image, LLaVA does not
            if model_kind == "qwen2-vl-gguf" and multi_frame_context:
                import base64
                
                # Select 2-3 diverse frames from this scene
                num_context_frames = min(3, len(scene_frames))
                if num_context_frames > 1:
                    # Pick frames evenly distributed through the scene
                    indices = [int(i * len(scene_frames) / num_context_frames) for i in range(num_context_frames)]
                    context_frames = [scene_frames[i] for i in indices]
                else:
                    context_frames = scene_frames
                
                # Multi-frame prompt - simple and direct for scale model work
                cap_prompt = prompt or f"Describe what you see in these {num_context_frames} images from the same scene: the car part being worked on (seat, body, chassis), any tools visible (airbrush, brush, tweezers), paint colors, and what the hands are doing."
                
                # Build multi-image message
                content = [{"type": "text", "text": cap_prompt}]
                for ctx_frame in context_frames:
                    with open(ctx_frame["path"], "rb") as f:
                        image_data = base64.b64encode(f.read()).decode("utf-8")
                    content.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{image_data}"}})
                
                # Get scene-level caption from multiple views
                output = model.create_chat_completion(
                    ...
                    max_tokens=250,
                    temperature=0.2,  # Lower for consistent scene descriptions
                )
                
                scene_caption = output['choices'][0]['message']['content'].strip()
                
                # Apply scene caption to all frames in this scene
                for frame in scene_frames:
                    frame["caption"] = scene_caption
                    frame["caption_model"] = caption_model.get("model_name")
                    frame["caption_prompt"] = cap_prompt
                    frame["caption_method"] = f"multi-frame-{num_context_frames}"
                    frame_count += 1
                
                # Show detailed progress
                scene_time = time.time() - scene_start_time
                elapsed = time.time() - start_time
                avg_time = elapsed / scene_count if scene_count > 0 else 0
                remaining_scenes = total_scenes - scene_count
                eta_seconds = remaining_scenes * avg_time
                eta_min = int(eta_seconds / 60)
                eta_sec = int(eta_seconds % 60)
                
                print(f"\r   Scene {scene_count}/{total_scenes} | Frames: {frame_count}/{total_frames} | "
                      f"Time: {scene_time:.1f}s | ETA: {eta_min}m {eta_sec}s | "
                      f"Caption: {scene_caption[:60]}...", end="", flush=True)
                if scene_count % 10 == 0 or scene_count == total_scenes:
                    print()  # New line every 10 scenes
                    
            else:
                # Single-frame captioning fallback for non-GGUF or disabled multi-frame
                for idx, frame in enumerate(scene_frames):
                    img = Image.open(frame["path"]).convert("RGB")
                    frame_count += 1
                    
                    if model_kind in ["qwen2-vl-gguf", "llava-gguf"]:
                        import base64
                        
                        cap_prompt = prompt or "Describe what you see in this image: the car part, tools, paint colors, and what the hands are doing."
                        
                        with open(frame["path"], "rb") as f:
                            image_data = base64.b64encode(f.read()).decode("utf-8")
                        
                        # Suppress ALL C++ verbose output (stdout and stderr) at OS level
                        try:
                            ...
                            print(f"\n⚠️  Error during captioning frame {frame_count}: {e}")
                            import traceback
                            traceback.print_exc()
                            ...
                            frame["caption_model"] = caption_model.get("model_name")
                            frame["caption_prompt"] = cap_prompt
                            frame["caption_method"] = "single-frame-error"
                            continue
                        
                        # Debug: Check if output has the expected structure
                        if not output or 'choices' not in output:
                            print(f"\n⚠️  Unexpected output structure for frame {frame_count}: {type(output)}")
                            print(f"     Output keys: {output.keys() if isinstance(output, dict) else 'not a dict'}")
                            caption = "[No caption - unexpected output structure]"
                        else:
                            caption = output.get('choices', [{}])[0].get('message', {}).get('content', '').strip()
                            if not caption:
                                print(f"\n⚠️  Empty caption extracted for frame {frame_count}")
                        frame["caption"] = caption
                        frame["caption_model"] = caption_model.get("model_name")
                        frame["caption_prompt"] = cap_prompt
                        frame["caption_method"] = "single-frame"
                        
                        # Show detailed progress for single-frame GGUF
                        scene_time = time.time() - scene_start_time
                        elapsed = time.time() - start_time
                        avg_time = elapsed / frame_count if frame_count > 0 else 0
                        remaining_frames = total_frames - frame_count
                        eta_seconds = remaining_frames * avg_time
                        eta_min = int(eta_seconds / 60)
                        eta_sec = int(eta_seconds % 60)
                        
                        print(f"\r   Scene {scene_count}/{total_scenes} | Frame {frame_count}/{total_frames} ({idx+1}/{len(scene_frames)} in scene) | "
                              f"Time: {scene_time:.1f}s | Avg: {avg_time:.1f}s/frame | ETA: {eta_min}m {eta_sec}s", end="", flush=True)
                ...
                # Progress update for single-frame transformer mode
                if idx % 5 == 0 or (idx + 1) == len(scene_frames):
                    elapsed = time.time() - start_time
                    avg_time = elapsed / frame_count if frame_count > 0 else 0
                    remaining_frames = total_frames - frame_count
                    eta_seconds = remaining_frames * avg_time
                    eta_min = int(eta_seconds / 60)
                    eta_sec = int(eta_seconds % 60)
                    print(f"\r   Scene {scene_count}/{total_scenes} | Frame {frame_count}/{total_frames} | "
                          f"Avg: {avg_time:.1f}s/frame | ETA: {eta_min}m {eta_sec}s", end="", flush=True)
            
            # New line after each scene
            if model_kind not in ["qwen2-vl-gguf", "llava-gguf"]:
                print()
    ...
    avg_per_frame = total_time / frame_count if frame_count > 0 else 0
    print(f"\n   ✓ Captioning complete: {frame_count} frames from {total_scenes} scenes")
    print(f"   Total time: {total_time/60:.1f}m | Avg: {avg_per_frame:.1f}s/frame")
    return frames
...
def extract_frames_parallel(video_path, interval=2):
    ...
    def extract_frame(i, ts):
        ...
        # This prevents 2x2 grid slicing (reduces from 6 batches to 1-2 batches per frame)
        ...
        for future in as_completed(futures):
            ...
            if completed % 100 == 0 or completed == total:
                with lock:
                    ...
                    eta_min = int(eta_seconds / 60)
                    eta_sec = int(eta_seconds % 60)
                    print(f"      Progress: {completed}/{total} frames | Speed: {fps:.1f} fps | ETA: {eta_min}m {eta_sec}s")
...
def filter_frames_by_scenes(frames, scenes):
    """Return frames that fall within kept scene time ranges, with scene_id assigned."""
    ...
    for idx, scene in enumerate(scenes):
        ...
        if end < start:
            continue
        ranges.append((float(start), float(end), idx))  # Add scene index
    if not ranges:
        return []
    ...
    kept = []
    range_idx = 0
    current_start, current_end, scene_id = ranges[range_idx]
    for frame in frames:
        ...
        if current_start <= ts <= current_end:
            frame["scene_id"] = scene_id  # Assign scene ID to frame
            kept.append(frame)
    return kept
...
def process_video(video_path, output_dir, clip_model, resnet_model, sample_interval, object_detection_cfg=None, caption_model=None, captioning_cfg=None):
    ...
    # Caption all kept frames with the 13B model
    kept_frames = filter_frames_by_scenes(frames, scenes)
    print(f"\n📸 Captioning {len(kept_frames)} kept frames with LLaVA-1.6-vicuna-13B")
    ...
    cap_prompt = captioning_cfg.get(
        "prompt",
        "Describe exactly what is visible. Identify model car parts, tools, hands, and materials. Avoid guessing; if uncertain, say 'unclear'."
    )
    ...
    if cap_enabled:
        ...
        if caption_model is None:
            ...
            kept_frames = caption_keyframes(
                ...
                multi_frame_context=False  # LLaVA doesn't support multi-image
            )
